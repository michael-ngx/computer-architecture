...
static counter_t reg_ready_asm1_q1[MD_TOTAL_REGS];
...
static counter_t reg_ready_asm1_q2[MD_TOTAL_REGS];
static counter_t sim_num_RAW_hazard_q2_1cycle = 0;
static counter_t sim_num_RAW_hazard_q2_2cycle = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_1cycle",
		   "total number of RAW hazards with 1 cycle (q2)",
		   &sim_num_RAW_hazard_q2_1cycle, sim_num_RAW_hazard_q2_1cycle, NULL);
  
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2_2cycle",
		   "total number of RAW hazards with 2 cycle (q2)",
		   &sim_num_RAW_hazard_q2_2cycle, sim_num_RAW_hazard_q2_2cycle, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "1 + (sim_num_RAW_hazard_q1_1cycle / sim_num_insn) + (2 * sim_num_RAW_hazard_q1_2cycle / sim_num_insn)", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + (sim_num_RAW_hazard_q2_1cycle / sim_num_insn) + (2 * sim_num_RAW_hazard_q2_2cycle / sim_num_insn)", NULL);
	...
	dlite_main(regs.regs_PC, regs.regs_NPC, sim_num_insn, &regs, mem);
      ...
      for (i = 0; i < 3; i++) {
        if (r_in[i] != DNA && reg_ready_asm1_q1[r_in[i]] > sim_num_insn) {
          ...
          else if (reg_ready_asm1_q1[r_in[i]] - sim_num_insn == 2) {
            sim_num_RAW_hazard_q1_2cycle++;
          }
          sim_num_RAW_hazard_q1++;
          reg_ready_asm1_q1[r_in[i]] = sim_num_insn;  // Prevent duplicated counts. After this hazard, propagated stalls are implemented 
          break;
        }
      }

      if (r_out[0] != DNA)
        reg_ready_asm1_q1[r_out[0]] = sim_num_insn + 3;
      if (r_out[1] != DNA)
      ...
      for (i = 0; i < 3; i++) {
        if (r_in[i] != DNA && reg_ready_asm1_q2[r_in[i]] > sim_num_insn) {
          ...
          if (reg_ready_asm1_q2[r_in[i]] - sim_num_insn == 1) {
            sim_num_RAW_hazard_q2_1cycle++;
          }
          else if (reg_ready_asm1_q2[r_in[i]] - sim_num_insn == 2) {
            sim_num_RAW_hazard_q2_2cycle++;
          }
      ...
      // Load to use hazard --> 2 cycles stall required
      // Else, 1 cycle stall required
      if (r_out[0] != DNA)
        reg_ready_asm1_q2[r_out[0]] = sim_num_insn + (((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)) ? 3 : 2);
      if (r_out[1] != DNA)
        reg_ready_asm1_q2[r_out[1]] = sim_num_insn + (((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)) ? 3 : 2);
      /* ECE552 Assignment 1 - END CODE*/
